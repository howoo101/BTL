        ...
        <div class="card border-primary">
            <div class="card-body">
                <div class="text-primary font-weight-bold card-title">
                    ...
                    <h5 style="float:left"><a href="${path}/calendar?ci_companyName=${ci_companyName}"
                                              style="text-decoration: none"><i class="material-icons">
                        calendar_today
                    </i></a></h5>
                </div>
...
<script>
    ...
    const hireNameFilter = ["주식회사", "(주)"];
    for (let i = 0; i < hireNameFilter.length; i++) {
        hireName = hireName.replace(hireNameFilter[i], "");
    }
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data['jobs']['count']; i++) {
				...
				const hireNameFilter = ["주식회사", "(주)"];
				for (let i = 0; i < hireNameFilter.length; i++) {
					companyName = companyName.replace(hireNameFilter[i], "");
				}
